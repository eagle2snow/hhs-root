...
<body class="hold-transition skin-blue sidebar-mini">
								...
								<div class="box-body">
										...
										<div class="col-sm-6">
											<div class="cl pd-5 bg-1 bk-gray mt-20">
												<span class="l"><a href="javascript:;"
													...
													<button onclick="openPerRe('添加',90,90,'${adp}add.htm')"
														class="btn btn-primary radius">
														<i class="glyphicon glyphicon-plus"></i> 添加
													</button></span>   分类<select class="form-control select2" onchange="getData(1)"  style="width: 150px !important;" id="typeSelect">
														<option value="0">－－全部－－</option>
													<c:forEach items="${typeList}" var="m">
														<option ${type==m.id?'selected':''} value="${m.id}">${m.name}</option>
													</c:forEach>
												</select> <span class="r">共有数据：<strong id="count">${page.count}</strong>
													条
												</span>
											</div>
										</div>
											...
											<div id="example1_filter" class="dataTables_filter">
												<div class="text-c">
													<input type="text" name="" id="key" placeholder="名称、介绍等"
														...
														class="input-text">
													<button name="" id="btn" class="btn btn-success"
														type="submit" onclick="getData(1)">
									...
									<table class="table table-bordered table-striped">
										<thead>
											...
											<tr>
												<th width="25"><input name="checkAll" class="minimal"
													type="checkbox"></th>
												<th width="40">ID</th>
												<th>名称</th>
												<th>分类</th>
										...
										<tbody>
											...
											<c:forEach items="${page.list}" var="model">
												<tr>
													...
													<th>${model.askCount}</th>

													<td><javatime:format value="${model.createTime}"
	...
	<script>
		...
		function getData(indexPage){
		      ...
		      location.href = '${adp}list/'+type+"/"+indexPage+'/${page.sizePage}.htm?k='+k;
		}
		
		
		function edit(id){
				openPerRe("编辑自动回复", 90,90,'${adp}update/'+id+'.htm');
		}
