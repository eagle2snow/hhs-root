										...
										<div class="col-sm-6">
											<div class="cl pd-5 bg-1 bk-gray mt-20">
												<span class="l"><a href="javascript:;" onclick="delByIds('${adp}')" class="btn btn-danger radius"><i
														class="glyphicon glyphicon-trash"></i> 批量删除</a> <a href="${adp}add.htm" class="btn btn-primary radius"><i
														class="glyphicon glyphicon-plus"></i> 添加</a></span> <span class="r">共有数据：<strong id="count">${page.count}</strong> 条
												</span>
											</div>
										</div>
										<div class="col-sm-6">
											<div id="example1_filter" class="dataTables_filter">
												<div class="text-c">
													<input type="text" class="" name="" id="key" placeholder="名称、介绍等"
														onkeypress="if(event.keyCode==13) {btn.click();return false;}" value='${key}' style="width: 250px; height: 33px"
														class="input-text">
									...
									<table id="example1" class="table table-bordered table-striped">
										...
										<tbody>
											...
											<c:forEach items="${page.list}" var="model">
												<tr>
													<td><input type="checkbox" name="ids" class="minimal" value="${model.id}"></td>
													<td>${model.id}</td>
													<th><img src='${model.avatar}' width="50" height="50" /></th>
													<th>${model.name}</th>
													...
													<td class="td-status">
														<div class="switch switchevent" data-on="primary" lang="${model.id}" title="${model.enable}" data-on-label="开"
															data-off-label="关" data-off="info">
