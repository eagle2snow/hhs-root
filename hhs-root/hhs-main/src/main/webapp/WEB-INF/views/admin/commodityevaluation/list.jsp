										...
										<div class="cl pd-5 bg-1 bk-gray mt-20">
											<span class="l"><a href="javascript:;"
												onclick="delByIds('${adp}')" class="btn btn-danger radius"><i
													class="glyphicon glyphicon-trash"></i> 批量删除</a> </span> <span class="r">共有数据：<strong id="count">${page.count}</strong> 条
											</span>
										</div>
									</div>
									<div class="col-sm-6">
									...
									<tbody>
										...
										<tr>
											...
											<td>${model.id}</td>
											 ...
											 <th>${model.commodity.name}</th>
											
											 <th>${model.orderItem.name}</th>
											
											 <th>${model.user.name}</th>
											
											 <th>${model.commodityAppraise.name}</th>
											
											 <th>${model.level}</th>
											
											 <th>${model.evaluationDetails}</th>
											
											 <th>${model.evaluationTime}</th>
											
											 <th>${model.replydetails}</th>
											
											 <th>${model.replyTime}</th>
