			...
			<c:forEach items="${orders }" var='order'>
								...
								<div class="cartpro_cont">
									...
									<div class="cutd">
										...
										<c:if test="${order.status==10 and order.appraise==0}">
											<span onclick="comments(${item.id})" class="defaultlinebtn radiusbtn msbtn" ${order.appraise eq "1"? 'style="display:none"': ''} style="float: right; margin-right: 2rem;">即刻评价</span>
										</c:if>
				...
				<li ${order.status==10 and order.appraise==0 or order.status==8 or order.status==4 ? 'style="display:none;"': ''} style="background-color: #fff; margin-bottom: 1rem; height: 2.5rem; margin-top: -1rem; padding-top: 0.5rem;">
					...
					<c:if test="${order.status==1}">
						<span onclick="payMoney(${order.id})" class="defaultlinebtn radiusbtn msbtn" style="float: right; margin-right: 2rem;">马上付款</span>
					...
					<c:if test="${order.status==6}">
						<span <%-- onclick="exitGoods(${order.id})" --%>  class="defaultlinebtn radiusbtn msbtn"
