...
<body>
	...
	<ul class="mt16 orderlist">
		...
		<c:if test="${order.status==7}"><span  class="defaultlinebtn radiusbtn msbtn" style="margin-left:45%;">待卖家收货</span></c:if>
		
		
		<c:if test="${order.status==6}"><span <%-- onclick="exitGoods(${order.id})" --%>  class="defaultlinebtn radiusbtn msbtn" style="margin-left:45%;">我要退货</span></c:if>
		
		
		 <c:if test="${order.status==5}"><span  class="defaultlinebtn radiusbtn msbtn" style="margin-left:45%;">退换货中</span></c:if>
		  ...
		  <c:if test="${order.status==4 and order.appraise==1 }">
										...
										<span onclick="toBackOrder(${order.id})" class="defaultlinebtn radiusbtn msbtn">申请售后</span></c:if>
		...
		<c:if test="${order.status==2}"><span id="urgent" onclick="urgent(${order.id})"  class="defaultlinebtn radiusbtn msbtn" style="margin-left: 45%;">给我加急</span></c:if>
										</li>
